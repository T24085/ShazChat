...
class SettingsWindow(QtWidgets.QWidget):
    def __init__(self, app):
        ...
        player_form.addRow("Team sync", self.connection_status)
    ...
    def set_connection_state(self, state: str):
        ...
        label, color = labels.get(state, labels["offline"])
        self.connection_status.setText(
            f'<span style="color:{color}; font-size:16px;">●</span> '
            f'<b>{label}</b> <span style="color:#9FB0C2;">team timers and chat</span>'
        )
...
class CapTimerApp:
    ...
    def set_connection_state(self, state: str):
        """Keep team-sync feedback in Settings, away from the gameplay HUD."""
        self.connection_state = state if state in {"connected", "connecting", "reconnecting", "offline"} else "offline"
        self.settings.set_connection_state(self.connection_state)
